...
app.post("/api/fetchJobsByLocationAPI2", (request, response) => {
  ...
  console.log(
    "(fetchJobsByLocationAPI2) - Received location to find:",
    requestData
  );

  db1.find(
    { [`location.area.${position}`]: requestData },
    function (err, docs) {
      if (err) {
        console.error("fetchJobsByLocationAPI:", err);
        response.status(500).json({ error: "Internal Server Error" });
  ...
  db1.find({}, (err, items) => {
    ...
    const countSalaryOccurrences = () => {
      ...
      const result = salaryBand.reduce((acc, band, index) => {
        ...
        if (count > 0) {
          acc.push([`£${band[0]} - £${adjustedEnd}`, count]);
        }
...
app.get("/api/getTotalNUmberOFJobsAPI", (request, response) => {
  db1.find({}, (err, jobs) => {
    if (err) {
      ...
      console.log(
        "getTotalNUmberOFJobsAPI: Success - number of jobs -",
        numofjobs
      );
...
app.post("/api/fetchJobsByLocationAndKeywordsAPI", (request, response) => {
  ...
  console.log(
    "(fetchJobsByLocationAndKeywordsAPI) - Received location to find:",
    location
  );
...
app.post("/api/fetchJobsCardsContract_TimeAPI", (request, response) => {
  ...
  console.log(
    "(fetchJobsCardsContract_TimeAPI) - Received contract_time to find:",
    requestData
  );
...
app.post("/api/fetchJobsCardsJobTypesAPI_TimeAPI", (request, response) => {
  ...
  db1.find(
    { "category.label": { $regex: new RegExp(requestData, "i") } },
    function (err, docs) {
      if (err) {
...
app.post("/api/jobSearchWithParamatersAPI", (request, response) => {
  ...
  const location = request.body.location;
  const keyWords = request.body.title;
  const salaries = request.body.salary_max;
  ...
  if (location.length > 0) {
    andArray.push({ [`location.area.${position}`]: location });
  }

  if (keyWords.length > 0) {
    andArray.push({ title: { $regex: new RegExp(keyWords, "i") } });
  }

  if (salaries.length > 0) {
    orConditions = salaries.map(([start, end]) => ({
      salary_max: { $gte: start, $lte: end },
    }));
    andArray.push({ $or: orConditions });
  }

  // Construct the final query with $and
  const query = { $and: andArray };

  // Use the query to find documents in your MongoDB collection
  db1.find(query, function (err, docs) {
    if (err) {
